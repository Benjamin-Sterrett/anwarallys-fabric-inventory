...
- **Live deploy:** CURRENT as of 2026-04-27. Live deploy sha `adf1312` = main HEAD. Firestore Rules + indexes deployed (manual `firebase deploy --only firestore:rules,firestore:indexes` ran 2026-04-27 morning). `VITE_PUBLIC_HOST=anwarallysinventory.pages.dev` set in GH Actions repo variables. Chrome-agent full smoke test scheduled for off-peak hours later today after the next round of Claude-only tickets ships.
