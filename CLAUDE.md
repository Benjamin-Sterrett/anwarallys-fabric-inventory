...
## Current Status
- **Phase:** Wave 1 ~98% shipped — Schema + auto-deploy + data boundary + data-boundary follow-ups + Security Rules + Rules defense-in-depth + **Self-service Staff page (data + UI)** all merged. Only PRJ-781 (Auth UX sign-in widget) remains for Wave 1 completion.
- **Last:** Three PRs in one session — **PRJ-865+866** (PR #9 squash `3223f5a`, Rules defense-in-depth, clean Codex APPROVE), **PRJ-856 PR A** (PR #10 squash `fdac2fd`, staff data layer + admin gate, 4 mid-impl Codex rounds with HIGH findings fixed in-code, 1 lead-Codex P2 deferred under `owner_override`/PRJ-873), **PRJ-871 = PRJ-856 PR B** (PR #11 squash `f689420`, `/staff` page UI + route, mid-impl HIGH/MEDIUM fixed in-code, 1 lead-Codex P1 deferred under `owner_override`/PRJ-873). Bootstrap config (Cloudflare Pages env + Firestore `/config/admin` + Firebase Auth user + `/users/{uid}`) all done via CLI by lead before PR #10.
- **Done:** Scaffold + Linear project + Waves 0–6 + 7-LLM discovery/synthesis + locked architecture + 3 gating tickets + repo public + CI green + auto-deploy + schema + data boundary + data-boundary follow-ups + Security Rules + **Rules defense-in-depth** + **bootstrap admin config** + **Self-service Staff (data + UI)**.
- **Next:** (1) **PRJ-781** Auth UX — sign-in widget every user sees. The /staff page already gates on `subscribeToAuthState` so Shaaiz can use it the moment the sign-in widget exists. (2) **PRJ-873** firestore.rules `.lower()` normalization (durable fix for the email-casing parity issue both PR #10 + #11 deferred). (3) **PRJ-874** defense-in-depth: prevent admin self-deactivation in Rules (mid-impl Codex finding from PRJ-871, fixed client-side, Rules tightening pending). (4) PRJ-841 (Vitest) + PRJ-842 (ESLint). (5) PRJ-804 validation checklist (physical checks).
